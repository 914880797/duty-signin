export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!name || !name.trim()) return Response.json({ error: '姓名不能为空' }, { status: 400 });
    if (!duty_date) return Response.json({ error: '日期不能为空' }, { status: 400 });
    if (!duty_time) return Response.json({ error: '时段不能为空' }, { status: 400 });
    ...
    await env.DB.prepare(
      `INSERT INTO signin_records (name, duty_date, duty_time, group_id, created_at, ip_address) VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(trimmedName, duty_date, duty_time, group_id || null, created_at, ip).run();

    return Response.json({ success: true, name: trimmedName, duty_date, duty_time, created_at });
  } catch (e) {
